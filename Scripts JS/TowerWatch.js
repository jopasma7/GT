if (document.URL.match("mode=incomings&type=unignored&subtype=attacks")) {
    $("#incomings_table").find("tr").eq(0).find("th").last().after('<th>Detector de Plantas</th>');
    var url = "https://" + location.host + game_data.link_base_pure + "overview_villages&mode=buildings&group=0&page=-1",
            ...
            block = [],
            timesRun = 1,
            rows = Number($("#incomings_table").find("th").first().text().replace("Orden ", "").replace("(", "").replace(")", ""));
                            ...
                            $.ajax({
                                    ...
                                    success: function(datas) {
                                            ...
                                            if (towerCoords.length == 0) {
                                                    UI.ErrorMessage("No tienes ninguna torre en tus pueblos!", 5000)
                                            }
                                    },
                            })
                    },
    ...
    var doStuff = function() {
            $.ajax({
                    ...
                    success: function() {
                            ...
                            } else if (commandName.includes("Hacha") || commandName.includes("Hacha")) {
                                    var remainingFields = seconds / unitSpeed[1];
                            } else if (commandName.includes("Espía") || commandName.includes("Espia")) {
                            ...
                            }else if (commandName.includes("Noble")) {
                            ...
                            if (intersectionPoints.length == 0) {
                                    $("#incomings_table").find("tr").eq(timesRun).find("td").last().text("¡¡Inception salvaje!!").css({
                                            "font-weight": "bold",
                                            "color": "red"
                                    });
                                    ++timesRun
                            ...
                            } else if (commandName.includes("Hacha") || commandName.includes("Lanceiro")) {
                            ...
                            function clock(x) {
                                    ...
                                    function myClock() {
                                            ...
                                            if (sec < 0) {
                                                    ...
                                                    var time = "¡Apareció un Ciobi Salvaje!";
