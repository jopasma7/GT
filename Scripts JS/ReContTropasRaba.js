...
function fnExecuteScript(){
    try{
        ...
        var unitDesc = {
            "spear":"Lanceros",
            "sword":"Soldados con Espada",
            "axe":"Soldados con Hacha",
            "archer":"Arqueros",
            "spy":"Espías",
            "light":"Cabellería Ligera",
        ...
        function fnTranslate(id){
            ...
            var lang=typeof(translation[win.game_data.market]=="undefined")?"es":win.game_data.market;
        ...
        function fnGetTroopCount(){				
            ...
            var colCount = win.$('#units_table '+(gameVersion>=7.1)?'thead':'tbody:eq(0)');
            colCount = colCount[0].getElementsByTagName('th').length  - 2;
            ...
            win.$('#units_table tbody').each(function(row,eleRow){
                ...
                win.$(eleRow).find("td:gt(0):not(:has(>a))").each(function(cell,eleCell){
                    ...
                    if(cell%colCount){
                        //console.log("Cell: "+cell + " elemCel: " +parseInt(win.$(eleCell).text()||"0",10) + " colCount: "+colCount + "Floor" +Math.floor(cell/colCount));
                        /* Ignore In the village (your own + foreign) */
                        if(Math.floor(cell/colCount) != 1 && Math.floor(cell/colCount) != 4){
                            //console.log("Imprimiendo: "+parseInt(win.$(eleCell).text()||"0",10))
    ...
    */
            ...
            docSource += "\t\t\t\t\t\t\t<h2>" + fnTranslate(curGroup++) + "<sup><span style=\"font-size:small;\">" + strVersion + "</span></sup><sub><span style=\"font-weight:100;font-style:italic;text-decoration:none;font-size:x-small;\"><a href=\"http://www.crosstrigger.com\" target=\"_blank\"> by rabagalan73</a></span></sub></h2>\n";
            docSource += "\t\t\t\t\t\t\t<hr>\n";
